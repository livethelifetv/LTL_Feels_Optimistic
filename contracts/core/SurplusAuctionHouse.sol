...
contract BurningSurplusAuctionHouse {
    ...
    **/
    ...
    function increaseBidSize(uint256 id, uint256 amountToBuy, uint256 bid) external {
        ...
        require(bids[id].bidExpiry > block.timestamp || bids[id].bidExpiry == 0, "BurningSurplusAuctionHouse/bid-already-expired");
    ...
    function settleAuction(uint256 id) external {
        require(contractEnabled == 1, "BurningSurplusAuctionHouse/contract-not-enabled");
        require(bids[id].bidExpiry != 0 && (bids[id].bidExpiry < block.timestamp || bids[id].auctionDeadline < block.timestamp), "BurningSurplusAuctionHouse/not-finished");
...
contract RecyclingSurplusAuctionHouse {
    ...
    **/
    ...
    function increaseBidSize(uint256 id, uint256 amountToBuy, uint256 bid) external {
        ...
        require(bids[id].bidExpiry > block.timestamp || bids[id].bidExpiry == 0, "RecyclingSurplusAuctionHouse/bid-already-expired");
    ...
    function settleAuction(uint256 id) external {
        require(contractEnabled == 1, "RecyclingSurplusAuctionHouse/contract-not-enabled");
        require(bids[id].bidExpiry != 0 && (bids[id].bidExpiry < block.timestamp || bids[id].auctionDeadline < block.timestamp), "RecyclingSurplusAuctionHouse/not-finished");
        safeEngine.transferInternalCoins(address(this), bids[id].highBidder, bids[id].amountToSell);
...
contract PostSettlementSurplusAuctionHouse {
    ...
    **/
    ...
    function increaseBidSize(uint256 id, uint256 amountToBuy, uint256 bid) external {
        ...
        require(bids[id].bidExpiry > block.timestamp || bids[id].bidExpiry == 0, "PostSettlementSurplusAuctionHouse/bid-already-expired");
     ...
     */
    function settleAuction(uint256 id) external {
        require(bids[id].bidExpiry != 0 && (bids[id].bidExpiry < block.timestamp || bids[id].auctionDeadline < block.timestamp), "PostSettlementSurplusAuctionHouse/not-finished");
        safeEngine.transferInternalCoins(address(this), bids[id].highBidder, bids[id].amountToSell);
        protocolToken.burn(address(this), bids[id].bidAmount);
        delete bids[id];
        emit SettleAuction(id);
