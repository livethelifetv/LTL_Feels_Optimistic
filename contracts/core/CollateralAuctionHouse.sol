...
contract EnglishCollateralAuctionHouse {
    ...
    **/
    ...
    function increaseBidSize(uint256 id, uint256 amountToBuy, uint256 rad) external {
        ...
        require(bids[id].bidExpiry > block.timestamp || bids[id].bidExpiry == 0, "EnglishCollateralAuctionHouse/bid-already-expired");
    ...
    function settleAuction(uint256 id) external {
        require(bids[id].bidExpiry != 0 && (bids[id].bidExpiry < block.timestamp || bids[id].auctionDeadline < block.timestamp), "EnglishCollateralAuctionHouse/not-finished");
...
contract IncreasingDiscountCollateralAuctionHouse {
    ...
    **/
    ...
    function modifyParameters(bytes32 parameter, uint256 data) external isAuthorized {
        if (parameter == "minDiscount") {
            ...
            require(both(data > 0, uint256(uint48(-1)) > addUint256(block.timestamp, data)), "IncreasingDiscountCollateralAuctionHouse/invalid-update-rate-time");
    ...
    */
    function getNextCurrentDiscount(uint256 id) public view returns (uint256) {
        ...
        if (both(uint48(block.timestamp) < bids[id].discountIncreaseDeadline, bids[id].currentDiscount > bids[id].maxDiscount)) {
            ...
            bool doneUpdating          = both(uint48(block.timestamp) >= bids[id].discountIncreaseDeadline, bids[id].currentDiscount != bids[id].maxDiscount);
